...
# HOWEVER THE BEST PACKAGE USED HERE EMPLOYS A DIFFERENT DEFAULT PRIOR.
...
# region of practical equivalence (ROPE) for assessing group equality.
# placed on effect size (d). Selected for corrispondance with effects smaller than "small" by Cohen's guidelines (1988).
ROPE <- c(-0.2, 0.2)
...
# returns: 
# "A Bayesian BEST test (Kruschke, 2013) was used to compare differences in IAT D1 change scores between the flowers 
# condition and the insects condition. The model employed 5 parameters to describe the data: the means of both conditions 
# (μ1, μ2), the standard deviations of both conditions (σ1, σ2), and a shared normality parameter (ν). We employed the 
# default BEST prior, which is a noncommittal prior intended to have minimal impact on the posterior distribution. 
# Specifically, for sample yi in (y1, y2), μi = normal(M = mean(yi), SD = sd(yi)*5)), σi = gamma(Mo = sd(yi), 
# SD = sd(yi)*5), ν = gamma(M = 30, SD = 30). In order to allow us to assess group equality as well as group differences, 
# a region of practical equivalence (ROPE) was defined (negligible effect size: -0.2 < d < 0.2). The posterior probabilities 
